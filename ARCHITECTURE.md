...
Agent guidance has a separate dependency boundary from the Python runtime graph. Repository guidance routes a task to the installed Pinboard coordination skill before read-only work becomes repository writes and while accepted repository work awaits human disposition; the Pinboard delivery and intake skills remain specialized callers. The coordination skill owns the shared threshold for user-relevant process detail, human task-start routing, explicit review-actor language, current-responsibility routing, bounded coordination lifetime including preparation start, project-specific Git baseline inference and checkout confirmation, bounded checkout-risk assessment, reconciliation of late accepted direction with the current brief and candidate, and human-approved repository wrap-up. Specialized skills retain their distinctive evidence and blocking outcomes without creating separate narration policy, persistent checkout ownership, or wrap-up state.
...
`application` owns use-case sequencing, persistence contracts, read models, and cross-capability workflows. Ordinary use cases request purpose-specific stored facts, convert them into the narrower domain snapshots used by decisions, and commit only closed accepted mutations. Complete stored state is reserved for explicit validation, rebuild, and handover.

| Owner group | Responsibility |
| --- | --- |
| `stored_state.py` | Typed storage records and the complete read aggregate retained for explicit whole-project operations |
...
| `decision_projection.py`, `service.py` | Shared-index projection of purpose-specific stored collections into domain decision facts and locked mutation orchestration |
...
| `files/views.py` | Semantic-content-derived focus, per-item, per-attempt, and per-receipt history projections; selected refresh, complete rebuild, and exact removal of superseded aggregate views |
...
| `sqlite/store.py` | Complete runtime connection and write-transaction lifetime, purpose-specific public read capabilities, exhaustive accepted-mutation routing, project revision advancement, expected-result rollback selection, and compact affected-view effects |
| `sqlite/state.py` | Purpose-specific read-fact assembly, explicit complete `StoredWorkState` assembly and cross-record validation, project metadata, and keyed or bounded transition history |
...
| `work_views.py` | Exact selected live-attempt brief reads for post-commit generated-view refresh plus complete brief reads for explicit rebuild composition |
| `work_state.py`, `work_state_models.py`, `work_state_commands.py` | Fresh initialization, optional post-success repository-care and user-config advice, whole-work-root validation, root resolution, validation presentation, and repair commands |
...
`.codex/pinboard/state.sqlite3` owns project revision and host epoch; immutable work-item definition revisions; item, attempt, focus, dependency, requirement, and proposal state; coordination, preparation, and attempt authority; accepted artifact references; and transition history. Every retained attempt or preparation lease must have its exact current generation anchor before application code can consume it. Relational dependency rows are the query-efficient projection of the current definition and must match it exactly. An ordinary mutation opens a write transaction, selects the exact locked decision facts, updates only changed rows named by one accepted closed mutation, and advances the revision with one history receipt. Unchanged dependency sets are not deleted and reinserted. Revision and affected-row guards return typed stale-action or fencing rejection values. The transaction owner rolls back those expected failures; SQLite, persisted-invariant, and programming failures roll back and remain exceptional. Initialization creates the exact empty current schema directly; live state enters only through focused mutations.

### Immutable artifacts

Accepted requirements, briefs, results, reviews, and other evidence are immutable files below `.codex/pinboard/artifacts/`. Canonical v2 work briefs and independent brief reviews are strict JSON; SQLite stores each accepted artifact's kind, selector, revision, digest, and size. Attempts and history own the relationships they actually consume. The installed brief-publication path validates and canonicalizes a candidate, publishes or verifies the immutable revision once, and accepts that publisher-returned reference without rereading it, changing scheduling, or rebuilding views. Activation, resume, and dispatch read only their selected accepted references and reuse the verified bytes through validation, comparison, prompt rendering, and post-commit attempt-view refresh. Readers never discover accepted artifacts by scanning the artifact tree. The files do not independently own lifecycle state.

### Generated views

`.codex/pinboard/views/` is human-readable output derived from SQLite and its accepted artifact references. The projection consists of `current.md`, one `items/<item-id>.md` per item, one `attempts/<attempt-id>.md` per attempt, and one bounded `history/<history-id>.md` per transition receipt. Queue order remains visible through the ordered overview and each live item's position; Pinboard no longer maintains ever-growing aggregate `queue.md` or `history.md` files. Generated bytes include semantic revisions where the individual view presents them, but omit the unrelated global database revision. A live v2 attempt view contains a complete Markdown rendering of the canonical JSON brief; Pinboard is its only writer, and no runtime path reads it for brief semantics.

A successful SQLite commit is authoritative before refresh begins. For mutations whose entity projections can change, the write transaction compares locked operation facts and returns one compact effect containing the receipt, affected identities, and only the relational records required to render them. Preparation renewal therefore changes no item projection, and a live attempt whose accepted brief reference is unchanged changes no attempt projection. A borrowed-coordination wrapper adds the exact acquisition and release receipts around the transition's compact entity facts. Ordinary commands replace only the affected current, item, attempt, and new history-receipt paths. Unchanged selected files keep their bytes and filesystem identity through atomic byte comparison. If refresh fails, the command reports repair guidance without rolling back the accepted transition. The explicit `pinboard views rebuild` command reconstructs every current per-entity projection, removes only the two known legacy aggregate paths, and does not replace files whose bytes already match. Validation intentionally derives every expected view and reports missing, stale, or legacy generated output separately from authoritative defects.
...
`pinboard init` resolves the default `.codex/pinboard` root below the shared repository root and idempotently adds only `/.codex/pinboard/` to that repository's local Git exclude file. Invoking initialization from a linked worktree therefore reuses the repository ledger and does not create a worktree-local authority. It never edits a committed `.gitignore`, so unrelated `.codex` content remains visible. An explicit `--work-root` selects that exact path instead. Interface-owned work-state composition creates the SQLite schema and its empty generated projection when `state.sqlite3` is absent. When the database exists, it verifies that exact schema before reconciling the fixed publication staging path: a same-file staging alias left after publication is removed, while a different-file conflict is rejected without replacement. It then ensures the artifact directories exist without inspecting artifacts or rebuilding views; explicit validation and rebuild commands own repair. Only after a successful fresh initialization and its receipt, the outer work-state interface prints one optional repository-care pointer and may read the user Codex config to recommend `body_after_prefix` when that user default is absent. The pointer starts no skill or work, and neither effect writes Codex configuration. A trusted project's `.codex/config.toml` may override the user default. Resume and failure paths print neither effect; unreadable or malformed user config suppresses only the setting recommendation.

### Reads and validation

Status reads fixed-size metadata and grouped counts. Overview and parallel preview read the live graph. Exact item, authority, artifact-reference, and definition commands use keyed selectors, while definition history uses a caller-bounded newest-first page. Action selection and lifecycle decisions assemble current decision facts without materializing accumulated transition history, superseded definition revisions, unrelated artifacts, or prior authority generations. Expected absence or an unavailable selection returns a typed result. Nullable source, notes, and queue position remain null through application JSON projections; human-facing text and generated Markdown render explicit absence markers. Each selected SQLite row is converted directly into its declared stored-state record; explicit storage checks are reserved for row cardinality, canonical history JSON, and relationships spanning records. These reads never parse generated Markdown. Routine open verifies fixed-size schema identity and connection setup without whole-database integrity pragmas.

`pinboard validate` is intentionally complete-project work. It runs SQLite quick and foreign-key integrity checks, assembles and structurally validates complete stored state, verifies every accepted artifact reference once, reuses those bytes to validate live v2 brief identity and structure, keeps historical terminal brief bytes opaque, derives every expected generated view, and reports view drift separately. `pinboard views rebuild` likewise reconstructs every declared projection. These explicit costs do not run during ordinary commands.

`pinboard handover --json` opens one validated `StoredWorkState` snapshot, projects every admitted and pending project fact into the strict application-owned `pinboard-project-handover/v1` model, and lets the interface owner read and verify each referenced artifact exactly once through the filesystem adapter. Artifact suffixes outside the installed media vocabulary are rejected before any JSON is written. This explicit export intentionally visits the whole accepted artifact set. Only after the full package is materialized does the CLI write canonical JSON. The path does not read generated views or mutate SQLite, artifacts, lifecycle, focus, or authority.
