...
`pinboard tool-contract` is another state-independent read. It derives one complete index from the installed parser leaves, recursively expanded exact command union, and closed action family. A selected operation or action projects its purpose, mutation class, authority, subject, lifecycle precondition, generated strict schema, artifact contract, success postcondition, and retry semantics. Brief publication includes typed unresolved local and cross-boundary starters and relational rules generated beside the canonical brief records. The starters preserve only structural tags; semantic and identity values remain unresolved, so this discovery path cannot invent scope, authority, or verification.
