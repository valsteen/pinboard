...
MCP startup also uses a user-level Git-style config at `$XDG_CONFIG_HOME/pinboard/config`, or `~/.config/pinboard/config` when `XDG_CONFIG_HOME` is unset. On first use, Pinboard writes `[mcp] omitRegexLookarounds = true`; if the key is missing from an existing config, it writes that value there too. An explicit `false` is preserved. Change the value and reconnect the MCP server to change the schemas advertised to clients. An invalid or unwritable config stops startup with an error naming the file and setting.
