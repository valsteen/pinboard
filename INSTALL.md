...
MCP startup also uses user-level Git config at `$XDG_CONFIG_HOME/pinboard/config`, or `~/.config/pinboard/config` when `XDG_CONFIG_HOME` is unset. On first use, Pinboard writes `[mcp] omitRegexLookarounds = true` as one Git config key; it uses the same write when only the key is missing and preserves other keys. It then reads the stored value. An explicit `false` is preserved. Change the value and reconnect the MCP server to change the schemas advertised to clients. An invalid or unwritable config stops startup with an error naming the file and setting.

The enabled setting removes regex lookarounds only from advertised MCP input schemas. Pinboard still validates tool requests strictly. It is a temporary workaround for a schema rejection observed on one Claude Code through central LiteLLM to OpenAI-model route; the component and version that reject the schema are unknown. Set it to `false` if your route accepts the original patterns. The projection can be removed when affected setups accept those original schemas.

Project contributor traces use the ignored Git config at `<managed-project>/.pinboard/contributor-traces.config`. On first eligible use, Pinboard creates the file privately at mode `0600`, writes `[pinboard "unsafe_persist_exact_pinboard_traces"] mode = off` as one key, and reads the stored mode. An existing file missing that key gets the same write without losing item overrides. The setting, secret risk, retention, and diagnosis procedure are in [Diagnose Pinboard invocations during contributor work](CONTRIBUTING.md#diagnose-pinboard-invocations-during-contributor-work).
