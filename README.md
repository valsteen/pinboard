...
SQLite is the current ledger authority. Direct attempt- and preparation-authority status, current item definition, and bounded definition history select only their named facts; most other commands still read the complete typed state, then mutations commit only the relations named by one accepted change. State-independent commands use lightweight root selection and compose no store. Each database-backed invocation then resolves one explicit durable layout and composes one store from it. Routine opens verify schema identity without a whole-database scan, while explicit `pinboard validate` owns complete SQLite integrity checks. Stale or failed changes leave the prior ledger intact. Immutable artifacts retain long-form contracts and review evidence; a command that published such evidence before a later rejection reports that committed effect explicitly. Generated views are convenient projections, not fallback state. The [architecture map](ARCHITECTURE.md) explains package ownership, persistence boundaries, current scaling limits, and failure semantics for contributors and agents.
