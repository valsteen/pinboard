...
SQLite `sqlite-v5` is the current ledger authority. Direct attempt- and preparation-authority status, item status, current item definition, bounded definition history, attempt inspection, candidate-bound review jobs, and parallel preview with explicit `--item` selectors select only their named facts; status, overview, global action discovery, and empty-selection all-safe preview still read the complete typed state, while mutations commit only the relations named by one accepted change. Item status uses an item-leading attempts index, named-attempt reads use existing keys for the attempt, its live context, and accepted brief, and selected parallel preview uses existing keys for the requested current items, direct dependencies, open attempts, and applicable authority, so retained history for unrelated work does not enter those reads. State-independent commands use lightweight root selection and compose no store. Each database-backed invocation then resolves one explicit durable layout and composes one store from it. Routine opens verify schema identity without a whole-database scan, while explicit `pinboard validate` owns complete SQLite integrity checks. Older schemas are rejected without read-time migration. Stale or failed changes leave the prior ledger intact. Immutable artifacts retain long-form contracts and review evidence; a command that published such evidence before a later rejection reports that committed effect explicitly. Generated views are convenient projections, not fallback state. The [architecture map](ARCHITECTURE.md) explains package ownership, persistence boundaries, current scaling limits, and failure semantics for contributors and agents.
