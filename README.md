...
Exact invocation capture is off by default. Use `<launcher-root>/scripts/pinboard --capture-evidence <file> --safe-to-persist-exactly -- <normal arguments>` for one CLI process, or start a dedicated MCP server with `<launcher-root>/scripts/pinboard --mcp --capture-evidence-dir <existing-directory> --safe-to-persist-exactly`. Opt in only when the invocation and expected output are safe to store without redaction. CLI capture keeps sensitive staging beside the selected file and cannot wrap MCP or Claude hook startup; it retains exact argv, stdout, stderr and exit status, including a real signal interruption. MCP startup proves writable publication before accepting callbacks. MCP capture retains decoded request and validated result values, not transport bytes or earlier client events; strict result-validation failure preserves an unavailable record with the callback classification and commit reference. Pre-publication capture failure remains unavailable, while a post-publication synchronization warning retains the selected digest-and-size filename, and routine diagnostics cannot consume the bounded reporting budget reserved for either failure. Neither aftermath replaces the tool result or invites replay. These mode-`0600` files are private caller-owned evidence outside Pinboard's ledger until an existing authorized operation accepts an exact reference.
