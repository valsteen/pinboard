...
GENERATED_APPLICATION_MODULE = "pinboard.application._generated.replacement_projection"
...
def _validate_private_row_imports(family: ReplacementFamily, source_root: Path) -> None:
    ...
    for path in sorted(source_root.rglob("*.py")):
        ...
        for node in ast.walk(module):
            if not isinstance(node, ast.ImportFrom) or node.module != GENERATED_APPLICATION_MODULE:
                continue
            imported = private_rows.intersection(alias.name for alias in node.names)
            if imported:
                symbol = min(imported)
                raise DeclarationError(
                    f"replacement family: private generated row symbol {symbol} imported by {path.as_posix()}"
                )
