...
- **Inspect exact facts or one snapshot.** The command line decodes one exact leaf and first selects the source checkout, shared repository, and intended work directory without touching storage. State-independent commands stop there. A database-backed command then resolves the durable layout and database location once and receives one store for its complete workflow. Direct attempt- and preparation-authority status, item status, current item definition, bounded definition history, attempt inspection, candidate-bound review jobs, and parallel preview with explicit item selectors request only their named facts. Item status uses an item-leading attempts index and returns only selected-item fields across the application boundary. A named-attempt read uses one transaction and returns a closed terminal or nonterminal fact variant: terminal inspection carries only project revision and attempt/item identity, while nonterminal inspection requires exact live and Git/scope facts, item and current definition identity, direct live dependencies, and one accepted brief reference. Only the nonterminal inspection variant retains the decoded selected brief. Selected parallel preview uses one transaction for the current project revision and only the requested current items, current definitions, direct dependency states, open attempts, and applicable preparation or attempt authority. Item, preparation, and selected parallel preview sample time once to project observation-time expiry, while attempt status preserves retained persisted state. Status, overview, global action discovery, and empty-selection all-safe preview still read one complete SQLite snapshot and ask application code to project the requested view. Inspection neither refreshes generated files nor changes authority. `input-contract` is deliberately different: it describes static action semantics and payload shape from the selected action kind without opening project state.

- **Validate before repairing.** Routine database open checks fixed-size metadata and exact schema identity without scanning stored rows. `validate` additionally runs SQLite's complete integrity and foreign-key checks, verifies accepted brief content, every accepted artifact identity, and every replaceable generated view against the bytes its complete snapshot should produce. Authoritative defects are errors; missing or stale generated views are warnings because SQLite and accepted artifacts remain the source of truth. Validation only reports. `views rebuild` separately derives and replaces every queue, item, attempt, and history projection from one snapshot and verified brief content.
...
**Refresh and present afterward.** A failed replaceable-view refresh can warn without undoing the authoritative commit. Ordinary preparation start presents the exact claim returned by its transaction; lower-level authority commands reload latest state for their recovery-oriented presentation. Transition JSON preserves the committed revision and includes the affected attempt's derived continuation when available. A known attempt uses the same exact context reader as `attempt inspect`; item-to-attempt lookup and replaceable-view refresh remain complete-state paths. If continuation is unavailable, the transition remains committed and attempt inspection can be retried without replaying the mutation. The representative code repeats the same provenance story: the application owns locked selection, accepted decision, targeted mutation, and commit; refresh and presentation follow.
...
- **Application code** requests exact operation facts where supported, still reads complete stored state for remaining families, and projects an accepted decision into one targeted storage mutation. Configured locations and concrete stores stay explicit at the outer composition boundary rather than being rediscovered by use cases.
