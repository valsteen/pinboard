...
- **Inspect exact facts or one snapshot.** The command line decodes one exact leaf and first selects the source checkout, shared repository, and intended work directory without touching storage. State-independent commands stop there. A database-backed command then resolves the durable layout and database location once and receives one store for its complete workflow. Direct attempt- and preparation-authority status, item status, current item definition, and bounded definition history request only their named facts. Item status uses an item-leading attempts index and returns only selected-item fields across the application boundary. Item and preparation status sample time once to project observation-time expiry, while attempt status preserves retained persisted state. Other inspection families still read one complete SQLite snapshot and ask application code to project the requested view. Inspection neither refreshes generated files nor changes authority. `input-contract` is deliberately different: it describes static action semantics and payload shape from the selected action kind without opening project state.
