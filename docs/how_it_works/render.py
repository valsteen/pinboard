...
- **Inspect exact facts or a declared wider scope.** The command line decodes one exact leaf and first selects the source checkout, shared repository, and intended work directory without touching storage. State-independent commands stop there. A database-backed command resolves the durable layout and database location once, constructs one store, and passes required capabilities through its workflow. Named item, attempt, authority, action-ID, and selected-parallel operations request only their selected facts and direct decision relationships. Status, overview, global action discovery, and empty-selection all-safe preview intentionally range over the current live portfolio, but exclude retained definition history, terminal attempt bodies, artifacts, and transition receipts. Initialization, validation, view rebuild, and handover are the only explicit complete-project families. Inspection neither refreshes generated files nor changes authority. `input-contract` is deliberately different: it describes static action semantics and payload shape without opening project state.
...
2. **Reread, select, and commit.** The application opens one write transaction, reads the selected action context and direct relationships, reselects the exact legal action, decides it, and commits the change with the invoking task and host identity. A stale or illegal action returns without changing the ledger; JSON output may carry fresh same-subject alternatives from another exact read.
