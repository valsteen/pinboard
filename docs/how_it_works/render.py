...
3. **Prepare the brief.** One ordinary preparation start atomically selects the current ready definition and creates or transfers its renewable claim. Preparation freezes the accepted outcome, constraints, stable authorities, and explicit uncertainty in one canonical brief, then reviews the smallest evidence set that can challenge the planned change. The item does not become active yet.
...
7. **Review and close the same decision.** Current ledger state derives the next operation. For review, a read-only job binds the exact candidate, accepted brief owner, and current result evidence for a separate reviewer in the current coding-agent runtime. That reviewer examines changed and unclassified surfaces, reuses unchanged evidence, and widens when architecture, persistence, wire, lifecycle, dynamic consumers, or a demonstrated blind spot require it. The owning task processes the verdict, then executes only the repository disposition, verified disposable-worktree and local and remote branch cleanup, and terminal transition the human explicitly authorizes. One instruction may authorize that complete ordered sequence; a failed or uncertain step stops later destructive and terminal effects. Every workflow-ending reply reads exact item status and reports the canonical current work item and attempt states, so accepted implementation, repository integration, cleanup, and terminal `done` remain distinct.
