...
2. Before acquisition or implementation, follow the envelope's exact interface-owned verifier and read the verified immutable prompt bytes. Read the complete canonical brief, including its bootstrap, before startup. Require current state authority `sqlite-v6`; never infer it from generated views or archived files. Follow the envelope's startup inputs using the worker's trusted post-launch `CODEX_THREAD_ID` and verified runtime host. Connected MCP startup calls `pinboard_attempt_authority` with operation `acquire`, exact roots, attempt, task, host, and TTL, then selects the exact worker continuation through `pinboard_actions` with the returned lease and generation. Validate a supplied same-worker lease instead of replacing it.
