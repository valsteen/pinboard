...
Use the connected Pinboard MCP tools for item status, action discovery, attempt inspection, artifact verification, attempt authority, and lifecycle transition. Preparation authority, overview, proposal creation, and brief publication serve the owning coordination task through the same surface. Every call carries exact project and work roots; a transition also carries the freshly discovered receipt and structured exact leaf payload. If a required covered tool is unavailable, stop that operation and report the missing connection; do not substitute shell commands or temporary payload files. Dispatch, review-job, static contract discovery, source planning, validation, view repair, and candidate restoration remain CLI-only.
...
2. Before acquisition or implementation, verify the exact artifact identity from the `pinboard-native-agent-launch/v1` envelope through `pinboard_artifact_verify` with its exact roots and identity. Require `pinboard-verified-artifact-reference/v1`, then read the immutable prompt bytes exactly; stop if the accepted reference ID, selector, size, SHA-256, verification result, or bytes differ. Require the MCP state result's authority to be `sqlite-v6`; never infer current state from generated views or archived files. Read the launched worker's trusted task identity from `CODEX_THREAD_ID` and the verified prompt's runtime host, then call `pinboard_attempt_authority` with operation `acquire`, the named attempt, post-launch task, host, TTL, and roots. If an existing same-worker lease was supplied, validate it instead. Select the exact worker continuation through `pinboard_actions` with the returned lease and generation.
...
When reacquiring an attempt returned from review, require the correction-dispatch prompt produced from the exact selected return receipt and current independently reviewed source bytes. Use its rooted MCP acquisition and continuation inputs, then read the durable correction reason and `review.md` before editing. Keep the same accepted brief, branch, evidence, and attempt identity. Treat the earlier `result.md` as preserved history, not a current readiness claim; refresh it only after the corrected candidate is stable and all required checks pass.
