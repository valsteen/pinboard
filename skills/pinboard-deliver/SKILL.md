...
After the owning task submits the exact candidate, run `pinboard attempt inspect --attempt-id <attempt> --json` and follow its derived continuation. When it names review, render `pinboard review-job --attempt-id <attempt> --candidate-revision <candidate> --json` and launch its prompt unchanged as one fresh-context, candidate-read-only subagent. To reuse accepted checkpoint evidence, the outcome owner first runs `pinboard validate --json` and stops on failure, then adds the exact caller-selected `--checkpoint-history-id <history>`. On a correction round it adds the exact `--correction-history-id <history>` returned by the selected prior correction; current `review.md` is bound independently and may describe a newer returned candidate. The job binds the current candidate, canonical accepted brief, outcome owner, current nonempty `result.md`, and every selected historical byte and digest. The reviewer verifies those identities, compares every historical candidate independently, resolves prior findings, and records which evidence was reused, revalidated, or stale from changed relationships. Rendering is read-only and does not invoke validation, accept, or complete the candidate.
