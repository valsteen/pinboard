...
When the connected Pinboard MCP surface advertises a covered operation, use it instead of spawning the CLI. Delivery uses MCP for item status, action discovery, attempt inspection, artifact verification, attempt authority, and lifecycle transition. Preparation authority, overview, proposal creation, and brief publication are available to the owning coordination task through the same surface. Dispatch, review-job, static contract discovery, source planning, validation, view repair, and candidate restoration remain CLI-only until their own MCP tools are advertised. Every MCP call carries the exact project and work roots; a transition also carries the freshly discovered opaque receipt and exact leaf payload.
...
2. Before any acquisition or implementation, verify the exact artifact identity from the `pinboard-native-agent-launch/v1` envelope through MCP when advertised; otherwise run its exact rooted `artifact verify` command. Require `pinboard-verified-artifact-reference/v1`, then read the named immutable prompt bytes exactly; stop if the accepted reference ID, selector, size, SHA-256, verification result, or bytes differ. Then run `<launcher-root>/scripts/pinboard status --json` and require authority `sqlite-v6`. Stop if validation fails or another authority is reported; never infer current state from generated views or archived files. Read the launched worker's trusted task identity from `CODEX_THREAD_ID` and the runtime-selected host from the verified canonical prompt, then acquire attempt authority through MCP when advertised or the prompt's exact command otherwise. If the prompt instead supplies an already owned lease for a resumed same-worker continuation, validate it. Select worker actions through MCP when advertised or run `<launcher-root>/scripts/pinboard actions --role worker` with the resulting lease identity and fencing generation.
