...
1. Resolve this plugin's executable relative to this file as `../../scripts/pinboard`. The installed launcher uses the plugin's ready cached environment; a source checkout launcher may use the repository's locked uv environment. Do not construct an ad hoc uv command or cache override as the ordinary route.
...
3. If the default `.codex/pinboard` root is absent, report `WORKFLOW_UNAVAILABLE`. Initialize it with `pinboard init` only when the user explicitly requests coordinated-work setup. An explicit `--work-root` remains an exact user-selected exception. When a successful fresh initialization emits the repository-care pointer, relay it once in the user-facing result by presenting `$repository-readiness`, `$slop-cleanup`, and `$maintaining-agent-guidance` as optional next steps. The pointer and Codex setting line are advice only: do not invoke a skill, create work, edit configuration, infer that a repository-care recommendation applies, or infer the effective project setting from them.
...
5. Inspecting SQLite state needs no lease. For one project transition, prepare its semantic payload first, using `pinboard input-contract <action-kind> --json` when the fields are not already known. Query the exact action as Role.PROJECT, then run `pinboard transition` with the returned receipt plus the invoking task and host identity. SQLite checks and commits the complete transition atomically. The `close` convenience command applies the same direct project authority for terminal human decisions.
...
When an invocation, role, authority requirement, artifact shape, or retry rule is not already known, run `pinboard tool-contract --json`, then request only the returned exact operation or action selector. This contract is static and does not need an initialized ledger. Do not inspect Pinboard source, reconstruct a command from help prose, or carry a general argument namespace past the selected leaf. For brief publication, `pinboard tool-contract --operation brief/publish --json` supplies the generated schema, unresolved local and cross-boundary starters, and relational constraints. Choose the boundary from accepted work, fill every semantic `null` from accepted scope and reviewed evidence, and never publish an unresolved or invented value.
...
Project transitions are direct atomic SQLite changes authorized by the invoking task and host identity. Disjoint preparation leases reserve exact ready-item definitions before activation, and disjoint attempt leases authorize active item-local work concurrently. Expiry, release, revocation, and higher fencing generations invalidate retained leased actions.
...
Do not use or wake another Codex task to review or receive the same outcome. Dispatching work, clarifying or widening scope, or sending a prior message does not establish current responsibility. A separate task owns a genuinely independent outcome and reports in its own conversation; it is never a subordinate return path.
...
Do not end the owning turn merely because implementation started. End with a running worker only when the user explicitly requested background execution; report that review by a separate Codex reviewer remains pending and perform it before any acceptance transition.
...
At every workflow-ending reply after implementation, review, repository disposition, follow-up, or terminal confirmation, state whether Codex is finished. When a user action remains, invite exactly one applicable next action: review the result, merge or publish the accepted candidate, answer one unresolved follow-up, or confirm closure. Once terminal confirmation has completed the last action, state that no further action remains. Keep immediate artifact announcements visible, but do not treat them as this closure statement. Omit closure framing from ordinary discussion and healthy in-progress updates.
