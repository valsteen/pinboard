...
6. Preparation and attempt work use their own renewable leases. Read the exact item definition or attempt state before acquiring or renewing one, and forward the lease identity and generation returned by the supported command or action receipt. If a required precondition is unavailable through a supported read, preserve that missing path as a product defect instead of inventing a value or reading raw SQLite state.
...
1. Read `pinboard item definition --item-id <item> --json`, then acquire the item's initial preparation claim with that exact project revision, item subject revision, definition revision, digest, and the invoking task and host identity. Do this before loading implementation sources or compiling a brief. The item remains ready. Renew the exact preparation lease before it expires throughout source closure, compilation, publication, correction, and review. If work stops, release it. After expiry, release, or revocation, use `preparation transfer` with the supported exact lease preconditions, then re-read the definition and revalidate every applicable closed source receipt against the current checkout. Reuse prior preparation only when both the transferred pin and those receipts remain exact; otherwise discard stale draft or review evidence and rerun the affected source closure, brief publication, and review before activation.
