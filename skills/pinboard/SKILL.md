...
When a confirmed branch or base mismatch belongs to an active or paused attempt, publish a replacement brief for that same attempt and accepted definition, then use the supported `rebind-attempt:<attempt>` project action with its corrected branch, base revision, and accepted brief reference. Rebinding preserves the attempt's active or paused state and its evidence, fences the previous worker generation, and never changes the checkout. Acquire fresh attempt authority before continuation or dispatch in the corrected environment.
