...
4. For interrupted preparation or attempt work, inspect the corresponding authority `status`. Continue a supplied same-worker lease only after validating its exact holder and generation. Otherwise use preparation `start` or attempt `acquire` with the caller's own task and host. These operations select initial ownership or a permissible inactive transfer under lock; neither tool exposes a `transfer` leaf. A live conflicting holder rejects unchanged. A higher generation fences actions retained by the interrupted task.
