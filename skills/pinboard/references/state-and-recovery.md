...
Resolve the project through Git's shared common directory. A linked worktree therefore uses the primary checkout's `.pinboard`, not a competing ignored root. Default initialization adds only the anchored `/.pinboard/` entry to that shared repository's local Git exclude file; it does not edit committed ignore files or hide unrelated `.codex` content. An explicit `--work-root` remains at the exact selected path.
