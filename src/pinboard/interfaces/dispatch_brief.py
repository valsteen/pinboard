...
def _canonical_prompt(
    ...
    return (
        "Use the pinboard-deliver skill for this repository attempt.\n\n"
        ...
        "1. Resolve this worker's exact task/session identity after launch through the current runtime's "
        "runtime-adapters.md instructions in the pinboard-deliver skill. Do not invent an identity or reuse "
        "a pre-launch identity.\n"
        "2. Replace <exact-worker-task-id> below with that identity as one quoted literal argument, "
        "then acquire attempt authority:\n"
        f"   {launcher} --project-root {project_root} --work-root {work_root_argument} "
        f"attempt acquire --attempt-id {attempt} "
        f"--task-id <exact-worker-task-id> --host-id {host} --ttl-seconds {environment.lease_ttl_seconds} --json\n"
